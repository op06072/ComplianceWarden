  ...
  {},
  {
      ...
      [](Box const &root, IReport *out) {
        ...
        auto checkIntegrityPasp = [&](const Box &pasp) {
          ...
          for(auto &field : pasp.syms)
            if(
              strcmp(field.name, "size") && strcmp(field.name, "fourcc") && strcmp(field.name, "hSpacing") &&
              strcmp(field.name, "vSpacing"))
                                ...
                                for(auto &stsdChild : stblChild.children)
                                  if(isVisualSampleEntry(stsdChild.fourcc))
                                    for(auto &sampleEntryChild : stsdChild.children)
                                      if(
                                        sampleEntryChild.fourcc == FOURCC("clap") ||
                                        sampleEntryChild.fourcc == FOURCC("pasp")) {
              ...
              if(box.fourcc == fourCC) {
                if(!findBox(&box)) {
                  if(parent.fourcc != FOURCC("ipco")) /*ipco is also a valid parent*/
                  {
                    checkIntegrity(box);
                    out->warning(
                      "Unexpected '%s' position (parent is '%s')", toString(fourCC).c_str(),
                      toString(parent.fourcc).c_str());
                  }
                }
              } else
        ...
        auto check = [&](uint32_t itemId) {
          ...
          for(auto &span : spans) {
            if(span.first > filesize || span.first + span.second > filesize) {
              out->error(
                "Data offset overflow for Item_ID=%u: offset is %lld (pos=%lld,len=%lld) while "
                "file size is %llu",
            ...
            auto checkBox = [&](int64_t offset) {
              ...
              if(box.fourcc != FOURCC("mdat") && box.fourcc != FOURCC("idat"))
                out->error(
                  "Data offset %lld belongs to box \"%s\": expecting \"mdat\" or \"idat\"", offset,
                  toString(box.fourcc).c_str());
            };
    ...
    { "Section 8.1.1: Media Data box (track)\n"
      ...
      [](Box const &root, IReport *out) {
        ...
        auto check = [&](uint32_t trackId, uint64_t offset) {
          if(offset > filesize) {
            out->error(
              "Data offset overflow for trackID=%u: offset is %lld while file size is %llu", trackId, offset, filesize);
            return;
          }
          ...
          if(box.fourcc != FOURCC("mdat"))
            out->error(
              "Data offset %llu belongs to box \"%s\": expecting \"mdat\"", offset, toString(box.fourcc).c_str());
        };
        ...
        for(auto &ipma : itemIds) {
          ...
          for(auto itemId : ipma) {
            if(itemId <= lastItemId)
              out->error(
                "Each ItemPropertyAssociationBox shall be ordered by increasing item_ID (%u, last=%u)", itemId,
                lastItemId);
        ...
        for(auto &ipma : itemIds)
          for(auto itemId : ipma)
            if(std::find(itemIdsCount.begin(), itemIdsCount.end(), itemId) == itemIdsCount.end())
              ...
              out->error(
                "There shall be at most one occurrence of a given item_ID but item_ID=%u found "
                "several times",
                itemId);
      } },
    { "Section 8.11.14.1\n"
                  ...
                  if(iprpChild.fourcc == FOURCC("ipma")) {
                    ...
                    for(auto &sym : iprpChild.syms)
                      if(!strcmp(sym.name, "version"))
                        ...
                        if(
                          std::find(ipmas.begin(), ipmas.end(), std::pair<int64_t, int64_t>{ version, sym.value }) ==
                          ipmas.end())
                          ipmas.push_back({ version, sym.value });
                        else
                          out->error(
                            "There shall be at most one ipma with a given pair of values of "
                            "version "
                            "and flags but { version=%lld, flags=%lld } found several times",
                            version, sym.value);
                  ...
                  for(auto &iprpChild : metaChild.children)
                    if(iprpChild.fourcc == FOURCC("ipco"))
                      if(iprpChild.children.size() <= 127)
                        out->warning(
                          "'ipma' flags should be equal to 0 unless there are more than 127 "
                          "properties in"
                          " the ItemPropertyContainerBox (found %d)",
                          (int)iprpChild.children.size());
      } },
    { "Section 8.11.14.1\n"
                ...
                for(auto &iprpChild : metaChild.children)
                  if(iprpChild.fourcc == FOURCC("ipma")) {
                    ...
                    if(ipmaVersion > 0 && !itemIdsRequire32bits) {
                      ...
                      out->warning(
                        "'ipma' version 0 should be used unless 32-bit item_ID values are "
                        "needed (Item_IDs = { %s})",
                        itemIdsStr.c_str());
                    }
                  }
      ...
      [](Box const &root, IReport *out) {
        ...
        for(auto box : zeroSizedBoxes)
          if(box != &root.children[root.children.size() - 1])
            out->error(
              "Box '%s' has size 0 but is not last int the top-level container", toString(box->fourcc).c_str());
      } },
    { "Section 6.1.2\n"
    ...
    { "Table 1: box structure and arity\n"
      ...
      [](Box const &root, IReport *out) {
        ...
        boxCheck(
          root, out, { FOURCC("vmhd"), FOURCC("smhd"), FOURCC("hmhd"), FOURCC("sthd"), FOURCC("nmhd") },
          { FOURCC("minf") }, { 1, 1 });
        ...
        // boxCheck(root, out, { FOURCC("url "), FOURCC("urn ") }, { FOURCC("dinf") }, { 1, INT32_MAX });
        // //TODO: mixed arities cannot be expressed for now
      ...
      [](Box const &root, IReport *out) {
        ...
        if(moovs.size() == 1)
          if(moovs[0]->position < ftyps[0]->position)
            out->error(
              "MovieBox (position=%llu) placed before FileTypeBox (position=%llu) in file", moovs[0]->position,
              ftyps[0]->position);

        for(auto mdat : mdats)
          if(mdat->position < ftyps[0]->position)
            out->error(
              "MediaDataBox (position=%llu) placed before FileTypeBox (position=%llu) in file", mdat->position,
              ftyps[0]->position);

        for(auto free_ : frees)
          if(free_->position < ftyps[0]->position)
            out->error(
              "FreeSpaceBox (position=%llu) placed before FileTypeBox (position=%llu) in file", free_->position,
              ftyps[0]->position);
      } },
    { "Section 6.1.4\n"
