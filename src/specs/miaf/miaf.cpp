...
std::initializer_list<RuleDesc> rulesMiafGeneral = {
    ...
    } },
  {
    ...
    [](Box const &root, IReport *out) {
      ...
      auto strFound = [](bool found) { return found ? "found" : "not found"; };

      if(!foundMiaf || !foundMif1)
        out->error(
          "compatible_brands list shall contain 'miaf' (%s) and 'mif1' (%s)", strFound(foundMiaf), strFound(foundMif1));
    },
  },
  {
    "Section 7.2.1.4\n"
  ...
  { "Section 7.2.1.7\n"
    ...
    [](Box const &root, IReport *out) {
      struct ImageItem {
        enum Type { generic, coded, derived };
        Type type = generic;
      ...
      for(auto &ii : imageItems)
        if(ii.second.type == ImageItem::Type::coded && ii.second.constructionMethod == 1)
          ...
          ii.second.type == ImageItem::Type::derived &&
          (ii.second.constructionMethod != 0 && ii.second.constructionMethod != 1))
        ...
        for(auto &span : spans) {
          auto checkBox = [&](int64_t offset) {
            ...
            if(box.fourcc != FOURCC("mdat"))
              out->error(
                "Item bodies of coded image \"%s\" itemID=%u (offset=%lld) belongs to box \"%s\": "
                "expecting \"mdat\"",
      ...
      auto check = [&](uint32_t itemId, uint32_t fourcc) {
        ...
        for(auto &span : spans) {
          auto checkBox = [&](int64_t offset) {
            ...
            if(box.fourcc != FOURCC("mdat"))
              out->error(
                "[%s] itemID=%u: offset=%lld belongs to box \"%s\": expecting \"mdat\"", toString(fourcc).c_str(),
                itemId, offset, toString(box.fourcc).c_str());
          };
                ...
                if(iinfChild.fourcc == FOURCC("infe")) {
                  ...
                  for(auto &sym : iinfChild.syms) {
                    if(!strcmp(sym.name, "item_ID"))
                      ...
                      if(
                        isVisualSampleEntry(sym.value) // coded image item
                        || sym.value == FOURCC("iden") || sym.value == FOURCC("grid") ||
                        sym.value == FOURCC("iovl")) // derived image item
                  ...
                  for(auto &sym : iprpChild.syms) {
                    if(!strcmp(sym.name, "item_ID"))
                      ...
                      if(imageItemIdsIspeFound.find(localItemId) != imageItemIdsIspeFound.end())
                        if(
                          std::find(ispePropertyIndex.begin(), ispePropertyIndex.end(), sym.value) !=
                          ispePropertyIndex.end())
    ...
    [](Box const &root, IReport *out) {
      ...
      auto checkIntegrity = [&](const Box &clli) {
        ...
        for(auto &field : clli.syms)
          if(
            strcmp(field.name, "size") && strcmp(field.name, "fourcc") &&
  ...
  },
  {
    ...
    [](Box const &root, IReport *out) {
      ...
      if(!matrix.empty()) {
        // either a or c but not both shall be equal to 0x00010000 or 0xFFFF0000, while the remaining one of a
        // and c shall be equal to 0;
        if(matrix[0]) {
          if(matrix[0] != 0x00010000 && matrix[0] != 0xffff0000)
            out->error(
              "Matrix field of the TrackHeaderBox: \"a\" value (%llX) shall be 0x00010000 or 0xFFFF0000", matrix[0]);
          ...
          out->error(
            "Matrix field of the TrackHeaderBox: \"c\" value (%llX) shall be 0x00010000 or 0xFFFF0000 "
            "(\"a\" is 0)",
            matrix[3]);

        // either b or d but not both shall be equal to 0x00010000 or 0xFFFF0000, while the remaining one of b
        // and d shall be equal to 0;
        if(matrix[1]) {
          if(matrix[1] != 0x00010000 && matrix[1] != 0xffff0000)
            out->error(
              "Matrix field of the TrackHeaderBox: \"b\" value (%llX) shall be 0x00010000 or 0xFFFF0000", matrix[1]);
          ...
          out->error(
            "Matrix field of the TrackHeaderBox: \"d\" value (%llX) shall be 0x00010000 or 0xFFFF0000 "
            "(\"b\" is 0)",
            matrix[5]);
    ...
    [](Box const &root, IReport *out) {
      ...
      if(!durations.empty()) {
        ...
        for(auto d : durations)
          if(d != refDuration)
            out->error(
              "All tracks shall have the same duration: found (%lld) while first track duration is "
              "%lld (expressed in 'mvhd' timescale)",
              d, refDuration);
      }
    },
  },
  ...
  },
  {
    ...
    [](Box const &root, IReport *out) {
      ...
      if(!(firstPredicate ^ secondPredicate))
        out->warning(
          "There should be at most one single track of any given type (rule predicates: {%d, %d})", firstPredicate,
          secondPredicate);
    },
  },
  { "Section 8.6\n"
      ...
      for(auto &e : editLists) {
        if(e.mediaRates.size() == 2) {
          ...
          if(e.mediaRates[0] != 1 || e.mediaRates[1] != -1)
            out->error(
              "Two media edits found: media rates shall be { 1, -1 }, found { %d, %d }", e.mediaRates[0],
              e.mediaRates[1]);

          if(
            (e.mediaTimes[0] != e.mediaTimes[1] + e.mediaRates[1] * e.editDurations[1]) &&
                                ...
                                for(auto &sym : stblChild.syms)
                                  if(!strcmp(sym.name, "sample_number")) {
                                    if(sym.value - lastSync > 4)
                                      out->error(
                                        "Found edit list for reverse playback: "
                                        "sync sample delta "
                                        "(%lld - %lld) is more than 4",
                                        sym.value, lastSync);
  ...
  { "Section 7.4.6\n"
    ...
    [](Box const &root, IReport *out) {
                      ...
                      if(trefChild.fourcc == FOURCC("auxl"))
                        for(auto &sym : trefChild.syms)
                          if(!strcmp(sym.name, "track_IDs")) {
                            if(trackId)
                              out->error(
                                "Unexpected: found several 'tref' track_IDs (%u then %lld)", trackId, sym.value);
      ...
      for(auto &t : tracks) {
        auto getMediaTimes =
          [findTrackId](Box const &root, uint32_t fourcc, uint32_t trackId) -> std::vector<Box const *> {
        ...
        if(
          !compareBoxVector(decodingTimesMaster, decodingTimesAlpha) ||
          !compareBoxVector(compositionOffsetsMaster, compositionOffsetsAlpha) ||
          !compareBoxVector(editListsMaster, editListsAlpha))
          out->error(
            "Composition times for trackId=%u different from alpha plane trackId=%u", t.videoTrackId,
            t.alphaPlaneTrackId);
      }
    } },
  { "Section 7.3.9\n"
              ...
              for(auto &iinfChild : metaChild.children)
                if(iinfChild.fourcc == FOURCC("infe"))
                  for(auto &sym : iinfChild.syms) {
                    if(!strcmp(sym.name, "flags"))
                      if(sym.value & 1)
                        // ISOBMFF 8.11.6.1: (flags & 1) equal to 1 indicates that the item is not
                        // intended to be a part of the presentation
                        break;
                    ...
                    if(!strcmp(sym.name, "item_ID"))
                      ...
                      if(std::find(pixiIndex.begin(), pixiIndex.end(), sym.value) != pixiIndex.end()) {
                        if(std::find(displayable.begin(), displayable.end(), item_ID) != displayable.end())
                          ...
                          out->warning(
                            "The pixel information property should be associated with hidden "
                            "images "
                            "(item_ID=%d, property_index=%d)",
                            item_ID, (int)sym.value);
                      }
                  }
                }
                ...
                if(iprpChild.fourcc == FOURCC("ipma")) {
                  ...
                  auto check = [&]() {
                    ...
                    if(actualTProps.size() > expectedTProps.size()) {
                      out->error(
                        "Item_ID=%d: too many transformative properties (%d instead of maximum %d)", item_ID,
                        (int)actualTProps.size(), (int)expectedTProps.size());
                      return;
                    }
                    ...
                    for(size_t i = 0, j = 0; i < actualTProps.size() && j < expectedTProps.size(); ++i, ++j) {
                      ...
                      if(j + 1 == expectedTProps.size()) {
                        if(actualTProps[i] != expectedTProps[j])
                          out->error(
                            "Item_ID=%d property: expecting \"%s\" ({ %s}), got \"%s\" ({ %s})", item_ID,
                            toString(expectedTProps[j]).c_str(), expected.c_str(),
                            toString(actualTProps[i + 1]).c_str(), actual.c_str());
                        else if(i + 1 != actualTProps.size())
                          out->error(
                            "Item_ID=%d property: expecting { %s}, got { %s}", item_ID, expected.c_str(),
                            actual.c_str());
                      }
                    }
                  };
