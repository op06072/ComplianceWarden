...
namespace
{
bool visitDerivationsBackward(
  DerivationGraph &graph, uint32_t itemIdDst, std::list<uint32_t> &visited,
  std::function<void(const std::list<uint32_t> &)> onError)
{
  ...
  for(auto &c : graph.connections) {
    if(c.dst == itemIdDst) {
      if(
        c.dst == c.src || visited.size() > maxDerivations // cycles
...
void checkDerivation(
  Box const &root, IReport *out, uint32_t fourcc,
  std::function<void(uint32_t, std::vector<std::pair<int64_t /*offset*/, int64_t /*length*/>>)> check)
{
  ...
  for(auto &itemId : itemIds) {
    ...
    for(auto &span : spans) {
      if(span.first + span.second > (int64_t)root.size) {
        out->error(
          "Image data (itemID=%u): found offset %lld + size %lld while file size is only %llu\n", itemId, span.first,
          span.second, root.size);
        continue;
      }
    }
...
static const SpecDesc specHeif = {
  ...
  { "isobmff" },
  {
                        ...
                        if(trefChild.fourcc == FOURCC("auxl"))
                          for(auto &sym : trefChild.syms)
                            if(!strcmp(sym.name, "track_IDs")) {
                              if(trackId)
                                out->error(
                                  "Unexpected: found several 'tref' track_IDs (%u then "
                                  "%lld)",
                                  trackId, sym.value);
                                    ...
                                    for(auto &sampleEntryChild : stsdChild.children)
                                      if(sampleEntryChild.fourcc == FOURCC("auxi")) {
                                        if(!foundAuxi)
                                          ...
                                          out->error(
                                            "AuxiliaryTypeInfoBox ('auxi') is "
                                            "present several "
                                            "times (trackIDs: video=%u, aux=%u)",
                                            trackIdVideo, trackIdAuxl);
                                      }
                    }

                if(!foundAuxi)
                  out->error(
                    "AuxiliaryTypeInfoBox ('auxi') is absent (trackIDs: video=%u, aux=%u)", trackIdVideo, trackIdAuxl);
              }
      } },
    { "Section 6.2\n"
                ...
                for(auto &sym : metaChild.syms) {
                  ...
                  if(std::find(idenItemIds.begin(), idenItemIds.end(), sym.value) != idenItemIds.end())
                    if(!strcmp(sym.name, "extent_count"))
                      if(sym.value != 0)
                        out->error(
                          "The derived image item [ID=%u] shall have no item body (i.e. no "
                          "extents), found %lld",
                          itemId, sym.value);
                }
              }
              ...
              if(metaChild.fourcc == FOURCC("iref")) {
                ...
                for(auto &sym : metaChild.syms) {
                  ...
                  if(!strcmp(sym.name, "reference_count"))
                    if(sym.value != 1)
                      out->error(
                        "reference_count for the 'dimg' item reference of a 'iden' derived image "
                        "item (ID=%u) "
                        "shall be equal to 1: found %lld",
                        itemId, sym.value);
                }
              }
      } },
    { "Section 6.6.2.3.1\n"
      ...
      [](Box const &root, IReport *out) {
        ...
        auto check = [&](uint32_t itemId, std::vector<std::pair<int64_t /*offset*/, int64_t /*length*/>> spans) {
          ...
          if(spans.size() > 1)
            out->error(
              "[tile] ItemID=%u: multiple spans (%d) not handled. Only considering the first one.", itemId,
              (int)spans.size());

          if(spans[0].second < 2) {
            out->error(
              "[tile] ItemID=%u: not enough bytes to parse: %d instead of 2 to compute FieldLength.", itemId,
              spans[0].second);
            return;
          }
          ...
          if(spans[0].second < 4 + fieldLength * 2) {
            out->error(
              "[tile] ItemID=%u: not enough bytes to parse: %d instead of %d.", itemId, spans[0].second,
              4 + fieldLength * 2);
            return;
          }
          ...
          if(rows * columns != refCounts[itemId])
            out->error(
              "Tile [itemId=%u]: the value of reference_count(%d) shall be equal to "
              "rows(%d)*columns(%d)=%d",
          ...
          for(auto toItemId : item.second) {
            ...
            if(!visitDerivationsBackward(graph, toItemId, visitedBackward, onError)) {
              ...
              if(!isVisualSampleEntry(itemFourccs[finalItemId]))
                out->error(
                  "Tiles: coded image (ItemID=%u derived from ItemID=%u) has type \"%s\" which "
                  "doesn't seem to identify an input image. "
                  "Backward derivation graph from ItemID=%u: %s",
                  toItemId, finalItemId, toString(itemFourccs[finalItemId]).c_str(), toItemId,
                  graph.display(visitedBackward).c_str());
            }
          }
      } },
    { "Section 6.6.2.3.1\n"
            ...
            if(iref.first == gridItemId)
              for(auto itemId : iref.second) {
                if(resIspe.width == Resolution().width)
                  ...
                  out->error(
                    "Tiles [ItemId]: all input images shall have exactly the same width and height\n"
                    "but found %dx%d for itemID=%u in 'ispe'",
                    gridItemId, resIspe.width, resIspe.height, d.itemRes[itemId].width, d.itemRes[itemId].height,
                    itemId);
              }
        }
      } },
    { "Section 6.6.2.3.1: tiles\n"
      ...
      [](Box const &root, IReport *out) {
        ...
        auto check = [&](uint32_t itemId, std::vector<std::pair<int64_t /*offset*/, int64_t /*length*/>> spans) {
          ...
          if(spans.size() > 1)
            out->error(
              "[grid] ItemID=%u: multiple spans (%d) not handled. Only considering the first one.", itemId,
              (int)spans.size());

          if(spans[0].second < 2) {
            out->error(
              "[grid] ItemID=%u: not enough bytes to parse: %d instead of 2 to compute FieldLength.", itemId,
              spans[0].second);
            return;
          }
          ...
          if(spans[0].second < 4 + fieldLength * 2) {
            out->error(
              "[grid] ItemID=%u: not enough bytes to parse: %d instead of %d.", itemId, spans[0].second,
              4 + fieldLength * 2);
            return;
          }
        ...
        // ensure each grid item is covered: we know from previous tests that sizes are constant and
        // consistent
        auto d = getDerivationsInfo(root, FOURCC("dimg"));
            ...
            if(iref.first == gridItemId) {
              for(auto itemId : iref.second)
                if(d.itemRes[itemId].width * gridLayouts[gridItemId].second < gridResolutions[gridItemId].width)
                  out->error(
                    "grid (itemID=%u) width(%d) not covered by tile (ItemId=%u) "
                    "width(%d)*numColumns(%d)=%d",
                    gridItemId, itemId, gridResolutions[gridItemId].width, d.itemRes[itemId].width,
                    gridLayouts[gridItemId].second, d.itemRes[itemId].width * gridLayouts[gridItemId].second);
                else if(d.itemRes[itemId].height * gridLayouts[gridItemId].first < gridResolutions[gridItemId].height)
                  out->error(
                    "grid (itemID=%u) height(%d) not covered by tile (ItemId=%u) "
                    "height(%d)*numRows(%d)=%d",
                    gridItemId, itemId, gridResolutions[gridItemId].height, d.itemRes[itemId].height,
                    gridLayouts[gridItemId].first, d.itemRes[itemId].height * gridLayouts[gridItemId].first);
            }
      } },
    { "Section 7.2.1\n"
      ...
      [](Box const &root, IReport *out) {
        ...
        if(!matrix.empty()) {
          // either a or c but not both shall be equal to 0x00010000 or 0xFFFF0000, while the remaining one
          // of a and c shall be equal to 0;
          if(matrix[0]) {
            if(matrix[0] != 0x00010000 && matrix[0] != 0xffff0000)
              out->error(
                "Matrix field of the TrackHeaderBox: \"a\" value (%llX) shall be 0x00010000 or "
                "0xFFFF0000",
                matrix[0]);

            if(matrix[3])
              out->error(
                "Matrix field of the TrackHeaderBox: \"a\" (%llX) or \"c\" (%llX) shall be equal to 0", matrix[0],
                matrix[3]);
          } else if(matrix[3] != 0x00010000 && matrix[3] != 0xffff0000)
            out->error(
              "Matrix field of the TrackHeaderBox: \"c\" value (%llX) shall be 0x00010000 or "
              "0xFFFF0000 (\"a\" is 0)",
              matrix[3]);

          // either b or d but not both shall be equal to 0x00010000 or 0xFFFF0000, while the remaining one
          // of b and d shall be equal to 0;
          if(matrix[1]) {
            if(matrix[1] != 0x00010000 && matrix[1] != 0xffff0000)
              out->error(
                "Matrix field of the TrackHeaderBox: \"b\" value (%llX) shall be 0x00010000 or "
                "0xFFFF0000",
                matrix[1]);

            if(matrix[4])
              out->error(
                "Matrix field of the TrackHeaderBox: \"b\" (%llX) or \"d\" (%llX) shall be equal to 0", matrix[1],
                matrix[4]);
          } else if(matrix[4] != 0x00010000 && matrix[4] != 0xffff0000)
            out->error(
              "Matrix field of the TrackHeaderBox: \"d\" value (%llX) shall be 0x00010000 or "
              "0xFFFF0000 (\"b\" is 0)",
              matrix[5]);
                      ...
                      if(!strcmp(sym.name, "item_ID")) {
                        ...
                        if(sym.value /*1-based*/ > (int)properties.size()) {
                          ...
                          if(properties[sym.value - 1] == FOURCC("ispe"))
                            foundIspe = true;
                          else if(isTransformative(properties[sym.value - 1]) && !foundIspe)
                            out->error(
                              "Item ID=%u: transformative property \"%s\" (index=%lld) found "
                              "prior to \"ispe\"",
                                  ...
                                  if(isVisualSampleEntry(stsdChild.fourcc))
                                    for(auto &sampleEntryChild : stsdChild.children)
                                      if(sampleEntryChild.fourcc == FOURCC("ccst")) {
                                        ...
                                        sampleEntryChildSizeInBits +=
                                          sampleEntryChildSizeInBits % 8 ? 8 - sampleEntryChildSizeInBits % 8 : 0;

                                        if(sampleEntryChildSizeInBits / 8 != 16)
                                          out->error(
                                            "invalid 'ccst' size: %llu instead "
                                            "of 16",
                                            sampleEntryChild.size);

                                        for(auto sym : sampleEntryChild.syms)
                                          if(!strcmp(sym.name, "reserved"))
                                            if(sym.value != 0)
                                              out->error(
                                                "invalid 'ccst' reserved "
                                                "value shall be 0, found "
                                                "0x%llX",
                                                sym.value);
                                      }
                    }
                    ...
                    for(auto &sym : iinfChild.syms) {
                      if(!strcmp(sym.name, "item_ID"))
                        ...
                        if(itemId == primaryItemId) {
                          ...
                          if(
                            !isVisualSampleEntry(sym.value) // coded item
                            && sym.value != FOURCC("iden") && sym.value != FOURCC("grid") &&
                            sym.value != FOURCC("iovl")) // derivation
                            out->error(
                              "primary item (Item_ID=%u) is not coded image or a derived image "
                              "item (found item_type=\"%s\")",
                  ...
                  if(iinfChild.fourcc == FOURCC("infe"))
                    for(auto &sym : iinfChild.syms) {
                      if(!strcmp(sym.name, "flags"))
                        if(!(sym.value & 1))
                          // ISOBMFF 8.11.6.1: (flags & 1) equal to 1 indicates that the item is
                          // not intended to be a part of the presentation
                          break;
                ...
                for(auto &field : metaChild.syms) {
                  ...
                  if(!strcmp(field.name, "to_item_ID"))
                    if(std::find(thmbIds.begin(), thmbIds.end(), fromId) != thmbIds.end())
                      out->error(
                        "Thumbnail image is linked to another thumbnail image (from item_ID=%u to "
                        "item_ID=%u)",
                        fromId, field.value);
                }
              }
                ...
                for(auto &field : metaChild.syms) {
                  ...
                  if(!strcmp(field.name, "from_item_ID")) {
                    if(std::find(fromIds.begin(), fromIds.end(), field.value) == fromIds.end())
                      ...
                      out->error(
                        "The number of SingleItemTypeReferenceBoxes with the box type 'dimg' and "
                        "from_item_ID=%u found more than once",
                        field.value);
                  }
                }
              }
    ...
    { "Section 6.6.2.2.3\n"
      ...
      [](Box const &root, IReport *out) { checkDerivationVersion(root, out, FOURCC("iovl")); } },
    { "Section 6.6.2.2.3\n"
      "'grid' box: version shall be equal to 0",
      [](Box const &root, IReport *out) { checkDerivationVersion(root, out, FOURCC("grid")); } },
    { "Section 6.5.11.1\n"
      "essential shall be equal to 1 for an 'lsel' item property.",
      [](Box const &root, IReport *out) { checkEssential(root, out, FOURCC("lsel")); } },
    { "Section C.2, D.2, F.2, G.2\n"
      ...
      [](Box const &root, IReport *out) {
        ...
        if(!found)
          out->warning(
            "File extension for \"%s\" doesn't match: expecting one of '%s', got '%s'", filename.c_str(),
            expected.c_str(), ext.c_str());
      } },
    { "Section B.2.3.1\n"
      "essential shall be equal to 1 for an 'hvcC' item property",
      [](Box const &root, IReport *out) { checkEssential(root, out, FOURCC("hvcC")); } },
    { "Section B.2.3.3\n"
      "essential shall be equal to 1 for an 'lhvC' item property",
      [](Box const &root, IReport *out) { checkEssential(root, out, FOURCC("lhvC")); } },
    { "Section B.2.3.5.1\n"
      "essential shall be equal to 1 for an 'tols' item property",
      [](Box const &root, IReport *out) { checkEssential(root, out, FOURCC("tols")); } },
    { "Section E.2.3\n"
      "essential shall be equal to 1 for an 'avcC' item property",
      [](Box const &root, IReport *out) { checkEssential(root, out, FOURCC("avcC")); } },
    { "Section H.2.2\n"
      "essential shall be equal to 1 for an 'jpgC' item property",
      [](Box const &root, IReport *out) { checkEssential(root, out, FOURCC("jpgC")); } },
    { "Section 9.3.1.1"
    ...
    { "Section 6.4\n"
      ...
      [](Box const &root, IReport *out) {
        ...
        boxCheck(
          root, out, { FOURCC("pasp") }, { FOURCC("ipco") }, { 0, INT32_MAX }); // TODO: at most one per image item
        boxCheck(
          root, out, { FOURCC("colr") }, { FOURCC("ipco") }, { 0, INT32_MAX }); // TODO: at most one per image item
        boxCheck(
          root, out, { FOURCC("pixi") }, { FOURCC("ipco") }, { 0, INT32_MAX }); // TODO: at most one per image item
        boxCheck(
          root, out, { FOURCC("rloc") }, { FOURCC("ipco") },
